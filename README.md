...
| 评估维度 | 权重 | 优秀(90+) | 良好(70-89) | 及格(60-69) | 不及格(<60) |
|:---|:---:|:---|:---|:---|:---|
| **知识准确率** | 40% | ≥95% | 85-94% | 75-84% | <75% |
| **角色相似度** | 25% | ≥80% | 70-79% | 60-69% | <60% |
| **幻觉控制率** | 20% | ≤5% | 6-15% | 16-25% | >25% |
| **回答流畅度** | 15% | 自然流畅 | 基本通顺 | 略生硬 | 语无伦次 |

**总分 = 知识×40% + 角色×25% + 幻觉控制×20% + 流畅度×15%**

### 评估测试集

| 测试集 | 题目数 | 说明 |
|:---|:---:|:---|
| `test_knowledge.json` | 100 | 知识问答（职业/星级/阵营/技能/属性） |
| `test_roleplay.json` | 50 | 角色扮演（与真实台词对比相似度） |
| `test_hallucination.json` | 30 | 幻觉检测（问不存在的干员） |

### 运行评估

```bash
# 1. 模型预测（生成 predictions.json）
# 格式：{"K001": "模型回答", "K002": "...", ...}
...
python eval/evaluate.py \
    ...
    --fluency_score 85  # 人工给流畅度打分（0-100）
```
...
```json
{
  ...
  "roleplay": {
    "avg_similarity": 0.412
  },
  "hallucination": {
    "rejection_rate": 0.833,
